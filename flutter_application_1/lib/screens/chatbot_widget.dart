...
// }
import 'package:flutter/material.dart';
import '../services/chatbotService.dart'; // Ton service pour appeler l'API
...
class _ChatbotDialogState extends State<ChatbotDialog> {
  ...
  void initState() {
    super.initState();
    _chatBotService = ChatbotService(); // URL de ton Laravel
    _chatMessages.add(ChatMessage(
      text:
          '👋 Bonjour ! Je peux vous aider à consulter le stock ou l’état des commandes.\nPosez-moi une question !',
      isUser: false,
      timestamp: DateTime.now(),
    ));
  }
  ...
  void _sendMessage() async {
    ...
    setState(() {
      _isTyping = false;
      _chatMessages.add(ChatMessage(text: response, isUser: false, timestamp: DateTime.now()));
    });
    _scrollToBottom();
  }
