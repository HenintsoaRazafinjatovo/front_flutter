...
class _CreateMovementDialogState extends State<CreateMovementDialog> {
          ...
          children: [
            ...
            Expanded(
              child: RadioListTile<String>(
                title: Row(
                  children: const [
                  Icon(Icons.arrow_upward, color: Color(0xFFE53E3E), size: 20),
                  SizedBox(width: 8),
                  Text('Sortie'),
                  ],
                ),
      ...
      children: [
        ...
        DropdownButtonFormField<String>(
          ...
          items: widget.articles.map((article) => DropdownMenuItem(
                ...
                child: Text('${article.intitule} (Stock actuel: ${article.stockActuel ?? 0})'),
              )).toList(),
          onChanged: (value) {
            setState(() {
  ...
  void _createMovement() async {
    ...
    if (selectedMovementType == 'sortie') {
      if (quantity > (article.stockActuel ?? 0)) {
        showDialog(
          context: context,
          builder: (context) => AlertDialog(
            title: Row(
            ...
            content: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                Text('Article : ${article.intitule}'),
                const SizedBox(height: 8),
                Text('Stock actuel : ${article.stockActuel ?? 0}'),
                Text('Quantité demandée : $quantity'),
                const SizedBox(height: 16),
                Container(
